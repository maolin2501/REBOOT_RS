...
class JointTrajectoryPoint:
    ...
    q: np.ndarray
    ik_success: bool
    v: np.ndarray | None = None   # 关节速度 (nv,)，用于前馈；端点为 0
...
def track_trajectory(
    ...
    _fill_joint_velocities(model, result)
    return result


def _fill_joint_velocities(
    model: pin.Model, pts: List[JointTrajectoryPoint],
) -> None:
    """用中心差分填充各点的关节前馈速度（端点置 0，rest-to-rest）。

    采用 ``pin.difference`` 在切空间求 q[i+1] ⊖ q[i-1]，对连续/旋转关节
    均正确；速度 = 差分 / (t[i+1] - t[i-1])。
    """
    n = len(pts)
    for p in pts:
        p.v = np.zeros(model.nv)
    if n < 3:
        return
    for i in range(1, n - 1):
        dt = pts[i + 1].time - pts[i - 1].time
        if dt <= 0.0:
            continue
        dq = pin.difference(model, pts[i - 1].q, pts[i + 1].q)
        pts[i].v = dq / dt
