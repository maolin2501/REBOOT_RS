...
通过 Linux joystick 接口 ``/dev/input/jsX`` 在后台线程持续读取手柄事件，
轴值归一化到 ``[-1, 1]``，按钮存为 ``0/1``。移植自 EL-A3 SDK，与机械臂无关。
"""
...
logger = logging.getLogger("reBotArm.teleop.joystick")
...
class LinuxJoystick:
    ...
    在后台线程中持续读取事件并更新共享状态（``axes`` / ``buttons``）。
    """
