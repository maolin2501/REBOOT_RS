...
class RobotArm:
    ...
    def enable(self, vendor: Optional[str] = None,
        ...
        failed: List[str] = []
        vendor_of = {jc.name: jc.vendor for jc in self._joints}

        def _is_enabled(name: str, st) -> bool:
            """判定使能成功：
            - RobStride: 反馈中 status_code 在运行态恒为 0，不能作使能指示；
              收到有效反馈帧（st 非 None）即视为在线/已使能。
            - 其它厂商（Damiao 等）: status_code == 1 表示已使能。
            """
            if st is None:
                return False
            if vendor_of.get(name) == "robstride":
                return True
            return st.status_code == 1
    ...
    def disable(self, vendor: Optional[str] = None,
        ...
        def _is_disabled(name: str, st) -> bool:
            """RobStride 的 status_code 在反馈中不可靠（运行/停用均为 0），
            disable_all() 已下发即视为失能；其它厂商按 status_code == 0 判定。"""
            if vendor_of.get(name) == "robstride":
                return True
            return st is not None and st.status_code == 0
