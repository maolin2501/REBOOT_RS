...
## Gravity Compensation Calibration (3-Step)

Known link masses `m_i` → least-squares COM `r_i` → `config/inertia_calibrated.yaml`.

0. `PYTHONPATH=. python3 scripts/gen_calib_poses.py` — auto-generate ~40 limit/self-collision-checked seed poses (→ ~80 static samples)  
1. `python3 example/11_calib_plan_path.py` — offline path + full self-collision check on every sample & segment  
2. `python3 example/12_calib_collect.py` — on-robot static torque sampling  
3. `python3 example/13_calib_solve.py` — offline COM solve  
4. `python3 example/14_gravity_compensation_calibrated.py` — A/B verify (press `c`)

Collision detection uses Pinocchio's built-in coal (no extra install needed).

See [README_zh.md](README_zh.md) for full Chinese documentation.
